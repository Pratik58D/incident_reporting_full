...
export const getHazardIncident = async(req:Request , res : Response)=>{
    try {
        const result = await pool.query(`SELECT i.id AS incident_id ,i.title,i.reported_by, h.id AS hazard_id ,h.name AS hazard_name  FROM incident i JOIN hazard_categories h ON i.hazard_id =h.id ORDER BY i.created_at DESC `);       
